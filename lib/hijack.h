...
static void disable_wp(void *target) {
	...
	mprotect((void *)((uintptr_t)target & ~0xFFF), 8192,
			 PROT_WRITE | PROT_READ | PROT_EXEC);
}
static void enable_wp(void *target) {
	mprotect((void *)((uintptr_t)target & ~0xFFF), 8192, PROT_EXEC | PROT_READ);
}
...
sym_hook hijack_start(void *target, void *new) {
	...
	if ((uintptr_t) new > (uintptr_t)4 * 1024 * 1024) {
		hook.hijack_size = 16;
		/* NOTE
		push rax;
		movabs rax, $addr;
		...
		*/
		memcpy(hook.n_code, "\x50\x48\xB8\x00\x00\x00\x00\x00\x00\x00\x00"
							"\x48\x87\x04\x24\xC3",
			   16);
		*(uintptr_t *)(&hook.n_code[3]) = (uintptr_t) new;
	} else {
		hook.hijack_size = 6;
		// NOTE push $addr; ret
